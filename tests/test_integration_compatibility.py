...
def test_predict_with_velocity(mock_model_config):
    ...
    net = SPILSNet(
        ...
        internal_scaler_class=MinMaxScaler(),
    ...
    # Test predict with two arguments (backward compatibility)
    # If input_size is 10, then u and v should each be 5
    u_half = np.random.rand(input_dim // 2)
    v_half = np.random.rand(input_dim // 2)
    
    net.initialize_memory_variables()
    pred2 = net.predict(u_half, v_half)
    assert isinstance(pred2, np.ndarray)
    assert pred2.shape == (input_dim,)
    
    # Verify that predict(u_half, v_half) is equivalent to predict(concatenate((u_half, v_half)))
    net.initialize_memory_variables()
    pred_combined = net.predict(np.concatenate((u_half, v_half)))
    
    # They should be identical since it's the same internal logic
    np.testing.assert_array_equal(pred2, pred_combined)
